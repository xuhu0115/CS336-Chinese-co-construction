...
1. 预分词的主要任务是将原始文本切分成可统计、可合并的基础单元，例如字符、字节或Unicode片段。常见策略包括基于空格和标点的切分、按Unicode类别划分，或直接采用字节级切分。需要注意的是并不是所有的分词器都需要用户显式进行预分词。例如基于SentencePiece的分词器将标准化和预分词逻辑内置，因此无需在外部额外执行预分词步骤。
   ...
   **Uincode与UTF-8的联系**
      - Unicode就像给全世界所有字符发的“身份证号”，不管是英文A、汉字“中”、还是emoji 😄等不同类型的字符都在Unicode里有一个唯一编号比如A是U+0041，“中”是U+4E2D。但“身份证号”本身只是一个抽象编号，电脑不能直接存。
      - UTF-8就像把这个字符对应的“身份证号”写进电脑文件的具体方式。它规定这个字符的编号应该用几个字节、按什么规则写下来。英文常用字符在UTF-8中只需要1个字节，而中文通常需要3个字节。
        
      两者配合起来，电脑才能正确存储和传输文字。
...
相比之下，UTF-8字节级策略具有最强的通用性，它把任意文本统一拆为字节序列，从而从根本上消除“未登录词（OOV）”问题并覆盖任意字符集。但因为它以最细粒度开始，训练时通常需要更多轮的共现统计与合并来把零散字节压缩为紧凑且具语义的token，才能在Transformer计算效率与语义表征之间取得平衡。
...
BPE、WordPiece、Unigram、SentencePiece这四种迭代算法简要分析：
   - BPE基本思路：从字符、最小单元开始，**迭代合并训练语料中出现频次最高的相邻符号对**，把最常见的对合并为一个新子词，直到达到目标词表大小。BPE是一个**贪心按频次合并**的算法：它只看“哪个相邻对出现得最多”，并把该对合并进词表。优点是实现简单且能高效压缩高频片段；缺点是只基于频次，可能把某些语义紧密但总体频次不高的片段拆成碎片，造成所谓“碎片化”。
     示例：如果`th`与`e`经常分别出现在多种上下文里，BPE可能先合并`t`+`h`形成`th`，但不一定马上合并成`the`，因为 `the`作为整体的相邻对出现频次必须高于其他候选对，也就是可能出现最后的词表显示toekn组合为`th`以及`e`这两个独立组合。
   
   - WordPiece基本思路：与BPE类似地逐步构建词表，但**合并、选择准则用概率、似然或其他统计评分**来衡量合并是否能提升模型对训练语料的拟合。因此WordPiece在合并决策上比纯频次更“统计学化”——它倾向于选择能显著提升整体语言模型、编码质量的合并。这样得到的词表在某些场景下比纯BPE更稳健、更利于下游训练。
     要点：不是简单比较P(A,B)与P(A)+P(B)，而是用**似然增益（或等价的概率评分**作为合并依据。
   
   - Unigram基本思路：先构造一个较大的候选子词集合（包括多种可能的子词片段），然后把子词当作**概率模型的原子**，通过 EM（最大似然估计）来学习每个候选子词的概率分布。训练过程中，会**反复估计概率并剪枝掉低概率的候选子词**，直到词表收敛或达到期望大小。与 BPE/WordPiece 的“逐步合并”相反，Unigram 是一种**从大候选表中删减**以最大化语料似然的策略。优点是灵活且概率化，能自然处理歧义分割与不确定性；缺点是训练和候选表设计复杂些。
   
   - SentencePiece基本思路：这是一个**独立的工具、实现库**，目标是从**原始文本（raw text）直接训练子词模型，不需要外部预分词**（即不需要先按空格把句子切开）。SentencePiece支持两种底层算法：BPE与Unigram。实现细节：它将空格（词边界信息）编码为特殊字符（训练输出中常见的 '▁' 用于表示词首空格），从而可以把空格也作为建表对象之一。总结：SentencePiece 是“把训练流程封装好并支持 raw-text 输入 + BPE/Unigram 两种后端”的工具，而不是一种全新的分词算法本身。
