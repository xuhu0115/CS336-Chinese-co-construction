...
2. **迭代**
   ...
   - **BPE**：每次选择最高相出现次数的相邻字符或字符组合合并为新token，更新语料token编码。比如两个相邻的A、B字符或字符组，当只有AB同时出现的总次数在其他相邻字符组合的总出现次数比较多的时候才会合并为一个新token，这样可能会使token有碎片化风险，好比`th`与`e`共同出现的次数相较于其他组合出现次数很少，那么BPE会把`th`与`e`视为两个独立token[`th`，`e`]，不利于LLM后续理解计算。这个过程就很像是一个只关注统计次数的计数器。
   - **WordPiece**：可以看成是BPE的变体，这一种算法不依高频出现的相邻字符对，而是基于概率计算得到关联最大的字符组合形成新的token，也就是根据哪些字符一起出现的频率高来划分。比如对于A、B两个字符或者字符对而言，如果P(A、B同时出现的概率)>P(A单独出现)+P(B单独出现)，则说明可能两个合起来的意义大于分开两者，那么AB就会组合成为一个新token替代初始A、B，不管A、B单独出现的频率是否靠前，`th`与`e`共同出现的次数相较于其他组合出现次数很少但是有`th`的地方几乎有`e`的“身影”，那么WordPiece会把`th`与`e`视为合并为一个token`the`这样得到token表会比BPE得到的token表更灵活。这个相较算法于BPE更像是一个开始关注统计数据背后的数学意义的学者。
   - **Unigram**：根据子词概率进行EM更新即删除低概率或重估概率，更新候选词表。也就是根据所有可能的token组合对于整个初始token表的影响大小即对文本压缩率压缩力度贡献越大影响越大而决定从当前表中保留影响大的、移除影响小的，不像是前两个算法往词汇表中添加或替换，这个过程不是看频率了而是看对于LLM压缩文本信息贡献程度。很像是在统计数字规律中只看占比这一影响的学者。
   - **SentencePiece**：可使用BPE或Unigram模式，迭代优化概率或合并规则，同时自动处理未登录字符。一种无需经过外部预分词（第二步）集合多种分词算法的“工具箱”，其会在内部自动划分出单个token它会通过把空格先转化为特殊字符，然后根据特殊字符划分出单个字符这也是和其余3种迭代字词算法的区别就是它学习的对象多了空格等，以及后续其余3种算法中的一种或多个配合使用得到最优秀token映射表。
