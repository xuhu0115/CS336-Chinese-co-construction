...
这段代码展示了自回归模型LLM推理中，“偷看未来”会产生编码偏差，以及**前缀累加**如何保证因果性，差异向量（错误方式未使用累加 - 正确前缀累加）量化了未来信息泄露对每个token的影响，错误方式使用一次性全局计算 $K^T V$ ，在每步输出中引入了未来token的信息。结果显示：
  ...
  - 随着序列推进，中间token受到未来token的影响逐渐减少，偏差依次减小。
  - **最后一个token “😄”** 没有未来token，正确和错误方式输出完全一致，差异为零。

前缀累加是线性注意力在自回归任务中的核心技术，它通过增量更新在保持 $O(L)$ 计算复杂度的同时，严格保证因果性，防止未来信息泄露，从而确保每个`token`的表示仅依赖已生成的前文。因果掩码阻止模型看到未来，预测任务迫使模型根据前文推断下一个token，而注意力机制则帮助模型从前文抓取关键线索，三者协同作用使模型能够自动学习语言规律，实现高效且准确的自回归预测。
