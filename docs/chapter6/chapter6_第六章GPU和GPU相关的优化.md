...
右侧是一个矩阵和矩阵在内存中的位置，假设在右侧这个图表中，有一组线程试图从左向右进行访问。每个线程都会尝试加载第一个元素，然后在下一个时间步加载第二列的元素，接着是第三列、第四列，依此类推。按列遍历（线程访问连续地址）可以**完全合并**。如果内存不连续的话，这些访问完全无法合并，因为它们**读取的是不同的内存块段**。这意味着必须读取整块内存才能执行任何操作。相反，如果按列方向访问，所有线程都会在同一个内存块段内读取，这样只需执行一次内存读取操作就能一次性获取所有数据。这是非常底层的优化，但至关重要。如果内存遍历顺序完全错误，实际获得的内存访问速度会远低于预期。
